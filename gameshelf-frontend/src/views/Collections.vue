        ...
        <section ref="scrollContainer" @scroll="handleScroll"
                    ...
                    <details v-for="collection in collections" :key="collection.id"
                        ...
                        <summary
                            ...
                            <div class="flex items-center gap-4">
                                <span
                                    class="text-gsmenta transform group-open:rotate-90 transition-transform duration-200">▶</span>
                        ...
                        <div class="p-5 border-t border-gsgris/10">
                            <div class="flex gap-6 overflow-x-auto pb-4 scrollbar-hide snap-x">
                                <div v-for="item in collection.items.slice(0, 10)" :key="item.id"
                                    class="min-w-[200px] max-w-[200px] snap-start">
                                    <GameCard v-if="!item.is_private" :game="item.game" />
                                </div>

                                <div v-if="collection.items.length > 10"
                                    class="min-w-[150px] flex items-center justify-center">
                                    <router-link :to="`/collections/${userId}/${collection.id}`"
                                        class="flex flex-col items-center gap-2 text-gsgris hover:text-gsmenta transition-colors">
                                        <div
                                            class="w-12 h-12 rounded-full border-2 border-dashed border-gsgris/40 flex items-center justify-center">
            ...
            <Transition name="fade">
                <button v-show="showButton" @click="scrollTop"
                    class="fixed bottom-8 right-8 z-50 p-3 rounded-md bg-gsmenta text-gsoscuro shadow-xl hover:bg-gsbosque hover:scale-110 transition-all">
