...
class LibraryItem(SoftDeleteModel):
    ...
    game = models.ForeignKey('games.Game', related_name='library_items', on_delete=models.CASCADE)
    ...
    status = models.CharField(max_length=3, choices=Status.choices, default=Status.PLANNING)
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=['library', 'game', 'platform'],
                condition=models.Q(deleted_at__isnull=True),
                name='unique_game_platform_per_library',
            )
        ]
